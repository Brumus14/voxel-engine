...
void *thread_pool_thread_main(void *arg) {
    ...
    struct mutex *tasks_lock = &pool->tasks_lock;
    struct condition *task_available = &pool->task_available;
    ...
    while (true) {
        ...
        while (priority_queue_is_empty(tasks)) {
            condition_wait(task_available, tasks_lock);
        }
...
void thread_pool_init(struct thread_pool *pool, unsigned int count) {
    ...
    mutex_init(&pool->tasks_lock);
    condition_init(&pool->task_available);
...
void thread_pool_destroy(struct thread_pool *pool) {
    ...
    condition_destroy(&pool->task_available);
    mutex_destroy(&pool->tasks_lock);
...
void thread_pool_schedule(struct thread_pool *pool,
    ...
    mutex_unlock(&pool->tasks_lock);

    condition_signal(&pool->task_available);
}
